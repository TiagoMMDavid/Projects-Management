...
private const val GET_LABELS_FROM_PROJECT_QUERY = "$GET_LABELS_BASE WHERE project_id = :pid"
private const val GET_LABELS_COUNT_FROM_PROJECT = "SELECT COUNT(lid) as count FROM LABEL WHERE project = :pid"
private const val GET_LABELS_FROM_PROJECT_EXCLUDING_ISSUE_QUERY =
    "$GET_LABELS_BASE WHERE project_id = :pid AND lid NOT IN (SELECT lid FROM ISSUE_LABEL WHERE iid=:issueId)"
private const val GET_LABELS_COUNT_FROM_PROJECT_EXCLUDING_ISSUE =
    "SELECT COUNT(lid) as count FROM LABEL WHERE project = :pid AND lid NOT IN (SELECT lid FROM ISSUE_LABEL WHERE iid=:issueId)"
...
class LabelsDb(
    ...
    fun getAllLabelsFromProject(page: Int, perPage: Int, projectId: Int, searchLabel: String?, issueIdToExclude: Int?): List<Label> {
        ...
        val query = buildString {
            if (issueIdToExclude != null) {
                binds["issueId"] = issueIdToExclude
                append(GET_LABELS_FROM_PROJECT_EXCLUDING_ISSUE_QUERY)
            }
            else {
                append(GET_LABELS_FROM_PROJECT_QUERY)
            }

            if (searchLabel == null) {
                append(" ORDER BY number")
            } else {
                binds["search"] = searchLabel
                append(" AND SIMILARITY(name, :search) > 0 ORDER BY SIMILARITY(name, :search) DESC")
            }
        }

        return jdbi.getList(query, Label::class.java, page, perPage, binds)
    }

    fun getLabelsCountFromProject(projectId: Int, searchLabel: String?, issueIdToExclude: Int?): Int {
        val binds = mutableMapOf<String, Any>("pid" to projectId)

        val query = buildString {
            if (issueIdToExclude != null) {
                binds["issueId"] = issueIdToExclude
                append(GET_LABELS_COUNT_FROM_PROJECT_EXCLUDING_ISSUE)
            }
            else {
                append(GET_LABELS_COUNT_FROM_PROJECT)
            }
